...
class CSSv1Handler(MyRedis):

    def __init__(self, app):
        super().__init__(asab.Config)
        # self.CSSv1Service = app.get_service("css.CSSv1Service")
        self.storage = app.get_service("asab.StorageService")

    async def set_configuration(self):
        # Initialize CSSv1 configuration
        print("\n[%s] Initialize CSSv1 configuration" % get_current_time())
    ...
    async def start(self):
        print("\n[%s] CSSv1Handler try to consume the published data from [Object Detection Service]" % get_current_time())
