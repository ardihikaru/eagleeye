...
docker run --name ews-service -d \
  ...
  5g-dive/eagleeye/web-service:2.2
```
...
docker run --runtime=nvidia --name detection-service-1 -d \
  ...
  5g-dive/eagleeye/dual-object-detection-service:2.2
```
...
docker run --name scheduler-service -d \
  ...
  5g-dive/eagleeye/scheduler-service:2.2
```
...
docker run --name visualizer-service -d \
  ...
  5g-dive/eagleeye/visualizer-service:2.2
```
