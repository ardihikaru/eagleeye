...
def getPersistenceBatch():
	...
	checkBatch = np.array([1, 2, 3, 4, 4, 3, 4, 8, 9, 10])
...
def persistenceModule(checkBatch):
	...
	while persistenceCount < PERSISTENCE_WINDOW - 1:
		...
		# We expect currentFrameID > startFrameID
		# 	then check next frame to see whether 'Flag' object is stil detected
		if (currentFrameID < startFrameID) or ((currentFrameID + 1) != nextFrameID):
			toleranceCount += 1
	...
	if toleranceCount <= TOLERANCE_LIMIT:
		print("[DBG] 	PiH Detected!")
		#sendFrameID(startFrameID)
