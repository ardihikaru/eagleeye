...
class SchedulingPolicyService(asab.Service):
    ...
    async def init_available_nodes(self, avail_nodes):
        ...
        for node in avail_nodes:
            ...
            self.avail_nodes.append(np_node)
    ...
    async def round_robin(self):
        ...
        # TODO: check the status of selected node_id
        t0_shm = time.time()
        await self._wait_until_ready(self.selected_node_id)
        t1_shm = time.time() - t0_shm
        print('\nLatency [Creating shm variable] in: (%.5fs)' % (t1_shm * 1000))

        # Set selected node as busy (idle=False)
        self.avail_nodes[self.selected_node_id]["idle"] = False
