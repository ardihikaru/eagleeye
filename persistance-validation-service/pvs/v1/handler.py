...
class PVSv1Handler(MyRedis):

    def __init__(self, app):
        super().__init__(asab.Config)
        # self.PVSv1Service = app.get_service("pvs.PVSv1Service")
        self.storage = app.get_service("asab.StorageService")

    async def set_configuration(self):
        # Initialize PVSv1 configuration
        print("\n[%s] Initialize PVSv1 configuration" % get_current_time())
    ...
    async def start(self):
        print("\n[%s] PVSv1Handler try to consume the published data from [Object Detection Service]" % get_current_time())
