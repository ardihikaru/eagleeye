...
from my_logging.log import Logging
from my_logging.task import TaskService
from missing_asab_components.docker import running_in_docker
...
class Application(metaclass=Singleton):
	...
	def __init__(self, args=None):
		...
		# custom var
		if "commons" in asab.Config:
			self.max_waiting = asab.Config["commons"].getfloat("max_waiting", 30.0)
		else:
			self.max_waiting = 30.0  # default value
		...
		if running_in_docker():
			from missing_asab_components.docker import Module
	...
	async def _run_time_governor(self, future):
		timeout = asab.Config.getint('general', 'tick_period')
		try:
			...
			for cycle_no in itertools.count(1):
				try:
					...
					if (cycle_no % 2) == 0:
						self.PubSub.publish("Application.tick/2!")
					if (cycle_no % 3) == 0:
						self.PubSub.publish("Application.tick/3!")
					if (cycle_no % 4) == 0:
						self.PubSub.publish("Application.tick/4!")
					if (cycle_no % 5) == 0:
						self.PubSub.publish("Application.tick/5!")

					# set dynamic value
					if (cycle_no % self.max_waiting) == 0:
						self.PubSub.publish("Application.tick/{}!".format(str(self.max_waiting)))
